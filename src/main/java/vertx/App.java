...
//  private static final Buffer HELLO_WORLD_BUFFER = Buffer.factory.directBuffer(HELLO_WORLD, "UTF-8");
    ...
    public void start() throws Exception {
        ...
        int port = (config.containsKey("port")) ? config.getInteger("port") : 8080;
        ...
        Pattern userPassPattern = Pattern.compile("//([\\w]+)\\:([\\w]+)\\@([\\w|\\.|\\-]+)\\:([\\w]+)/([\\w]+)");
        Matcher findUserPass = userPassPattern.matcher(env.get("DATABASE_URL"));
        String dbUser = "";
        String dbPass = "";
        String dbHost = "";
        String dbPort = "";
        String dbName = "";
        while (findUserPass.find()) {
            dbUser = findUserPass.group(1);
            dbPass = findUserPass.group(2);
            dbHost = findUserPass.group(3);
            dbPort = findUserPass.group(4);
            dbName = findUserPass.group(5);
        }
        PgConnectOptions options = new PgPoolOptions();
        options.setDatabase(dbName);
        options.setHost(dbHost);
        options.setPort(Integer.valueOf(dbPort));
        options.setUser(dbUser);
        options.setPassword(dbPass);
        options.setCachePreparedStatements(true);
        options.setSslMode(SslMode.PREFER);
        client = PgClient.pool(vertx, env.get("DATABASE_URL")); // + "?ssl=true&sslmode=prefer&sslfactory=org.postgresql.ssl.NonValidatingFactory&sslfactory=org.postgresql.ssl.NonValidatingFactory"); //vertx, new PgPoolOptions(options).setMaxSize(1));
        ...
        client.preparedQuery(SELECT_FORTUNE, ar -> {
            ...
            if (ar.succeeded()) {
                ...
                response
                        ...
                        .end(templates.vertx.FortunesTemplate.template(fortunes).render(factory).buffer());
            } else {
    ...
    public static void main(String[] args) throws Exception {
        ...
        vertx.deployVerticle(App.class.getName(),
                new DeploymentOptions().setInstances(VertxOptions.DEFAULT_EVENT_LOOP_POOL_SIZE).setConfig(config), event -> {
                    if (event.succeeded()) {
                        logger.info("Server listening on port " + ((config.containsKey("port")) ? config.getInteger("port") : 8080));
                    } else {
