...
cat(paste0('<img src="/post/', small_input, '" style="float: right; width:256px; margin-top: -128px; padding: 1%;" />'))
```
...
```{r idx_data, cache = FALSE, echo = TRUE}
...
```{r normal_preview, cache = FALSE, echo = FALSE, results = "asis"}
heights <- map$heights
...
```{r idx_data_2, cache = FALSE, echo = TRUE}
currentRow <- 1
indices <- c()
...
```{r idx_knit_for_loop, cache = FALSE, echo = TRUE}
...
Finally, all of this data must be saved to json and then stitched together into a JS module which defines a new `terrain` (heightmap) object that can be imported by `@sveltejs/gl`, with the form:
<pre><code>
    new Geometry({
        position: {
