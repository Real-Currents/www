...
map <- mapTerrainCoords(image_test, nrow(image_test), ncol(image_test))
...
elmat %>%
    sphere_shade(zscale = 100, sunangle = sunangle, texture = "desert") %>%
...
```{r rayshader_high_dim, cache = FALSE, echo = TRUE, results = "asis"}
heights <- mapHeight(image_gray, length(image_gray[1,]), length(image_gray[, 1])) # stick with the simple method for now
#heights <- (mapTerrainCoords(image_gray, nrow(image_gray), ncol(image_gray)))$heights
...
image_shaded <- matrix(3, nrow = height, ncol = width)
        ...
        for (y in 1:quad_height) {
            for (x in 1:quad_width ) {
                if (
                    ...
                    (y > quad_width) ||
                    (x > quad_height) ||
                    (quad_height_start + y > quad_height_end) ||
                    (quad_width_start + x > quad_width_end)
                ) {
                    print(paste0(quad_width_start + y, ", ", quad_height_start + x, " => ", x, ", ", y))
                }
        ...
        for (y in 1:quad_height) {
            for (x in 1:quad_width ) {
                if (
                    ...
                    print(paste0(y, ", ", x, " => ", quad_height_start + y, ", ", quad_width_start + x))
                }
                #image_shaded[quad_height_start + y, quad_width_start + x] <- c(
                #    (single_angle_shadow[y, x] * image_shaded[quad_height_start + y, quad_width_start + x, 1] / height_mult),
                #    (single_angle_shadow[y, x] * image_shaded[quad_height_start + y, quad_width_start + x, 2] / height_mult),
                #    (single_angle_shadow[y, x] * image_shaded[quad_height_start + y, quad_width_start + x, 3] / height_mult),
                #)
            }
        }
    }
...
I have made use of the `mapTerrainCoords` method that was developed in [Generating Planar Terrain Mesh](post/terrain) to generate the terrain heights and normals, and then write them out to a PNG and JSON file, respectively:
