...
In part 2, I continue to employ `mapTerrainCoords` and the ray-tracing shading methods provided by [rayshader](https://github.com/tylermorganwall/rayshader) that were explored in [Generating Planar Terrain Mesh](post/terrain) and [Events And Shade: Part 1](post/simulate-events-and-shade-1), respectively, to generate the terrain height maps, normal maps and shaded textures:
...
```{r rayshaded_map, cache = FALSE, echo = FALSE, results = "asis"}
xparts <- 2
yparts <- 2
width <- ncol(heights)
height <- nrow(heights)
